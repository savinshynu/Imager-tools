"""
Module to Filter slowly moving object like airplanes in the images
Author: S. S. Varghese
"""
...
def filter_planes(inf,mjd_day,stokes):
   """
   Filter an input list of candidates for airplane from the images
   Returns output array with filtered events
   
   Parameters:
   inf - input list of candidates, numpy array
   mjd_day - MJD day, float
   stokes - Stokes parameters, int between [0, 1, 2, 3] 
   """
   #stokes = 0 #For stokes I
   list_ap = np.zeros((inf.shape[0],9)) #making an array for airplanes
   num_ap = 0
   list_good = np.zeros((inf.shape[0],9)) # array for good candidates
   num_good = 0
   ...
   for hour in range(0,24):
      ...
      for filename1 in files1:
         ...
         #dset1=hf.get('image')
         tm_ind = hf.get('time') # collect time info
         header = hf.get('header') # collect header info
         head  =  header[0,:]
         ...
         # Going through each item in the input list
