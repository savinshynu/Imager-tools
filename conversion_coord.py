...
def eq2pix(ra, dec, MJD, h, m, s, size, psize, lat, lon, cent_az, cent_alt):
    """
    ra,dec is the right ascension and declination in degrees, then comes time in MJD,H,M,S.  size is the grid size usually 128, psize is pixel size in degrees, cent_az and cent_alt is the phase center values in degrees from the header,lat and lon are the station coordinates. If passing an array of ra,dec also pass an array of MJD,H,M,S
...
def eq2hrz(ra, dec, MJD, h, m, s, size, psize, lat, lon, cent_az, cent_alt):
   ...
   returns altitude and azimuth
    ...
    x = (x*((180.0/np.pi)/psize)) + (size/2.0) -1  # 63,63 center pixel is used 
    y = (y*((180.0/np.pi)/psize)) + (size/2.0) -1
    try:
       comp3 = np.where((Att < 0))[0]
       #print comp3
       x[comp3] = 1.0
...
# the same above code going from azimuth,altitude to pixels
...
# Using the sin projection where you can adjust the phase center. This one converts pixel coordinates to azimuth,altitude using sin projection which then maps to ra,dec values
def pix2eq(x, y, size, psize, MJD, h, m, s, lat, lon, cent_az, cent_alt):

    """
    x,y are the pixel cordinates, then comes time in MJD,H,M,S. size is the grid size usually 128, psize is pixel size in degrees, cent_az and cent_alt is the phase center values in degrees from the header, lat and lon are the station coordinates.
